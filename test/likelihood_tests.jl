...
@testset "Likelihood tests" begin
  ...
  ll = calculate_log_likelihood_at_parameter_point(protein_at_observations, small_delay_1, measurement_variance)
  @test ll ≈ readdlm(string(loading_path, "log_likelihood_value.csv"), ',')[2]

  ll = calculate_log_likelihood_at_parameter_point(protein_at_observations, small_delay_2, measurement_variance)
