...
function kalman_filter_state_space_initialisation(protein_at_observations,model_parameters,measurement_variance = 10)
    ...
    ####################################################################
    #
    state_space_mean_derivative = Array{Float64}(undef,(total_number_of_states,7,2));#zeros(total_number_of_states,7,2)
    #
    ...
    state_space_variance_derivative = Array{Float64}(undef,(7,2*total_number_of_states,2*total_number_of_states));#zeros((7,2*total_number_of_states,2*total_number_of_states));
    for diagonal_index in 1:initial_number_of_states
        state_space_variance_derivative[:,diagonal_index,diagonal_index] = initial_mRNA_scaling*state_space_mean_derivative[1,:,1]
        state_space_variance_derivative[:,diagonal_index+total_number_of_states,diagonal_index+total_number_of_states] = initial_protein_scaling*state_space_mean_derivative[1,:,2]
    end # for
    predicted_observation_mean_derivatives = Array{Float64}(undef,(number_of_observations,7,2));#(zeros((number_of_observations,7,2))
    predicted_observation_mean_derivatives[1,:,:] .= state_space_mean_derivative[initial_number_of_states,:,:]

    predicted_observation_variance_derivatives = Array{Float64}(undef,(number_of_observations,7,2,2));#zeros((number_of_observations,7,2,2))
    predicted_observation_variance_derivatives[1,:,:,:] .= state_space_variance_derivative[:,[initial_number_of_states,
...
function kalman_prediction_step(state_space_mean,
    ...
    ###
        ...
        state_space_variance_derivative[:,[next_time_index,
                                           total_number_of_states+next_time_index],
                                          [next_time_index,
                                           total_number_of_states+next_time_index]] .= next_covariance_derivative_matrix
        ...
        for intermediate_time_index in past_time_index:current_time_index
            ...
            covariance_matrix_derivative_intermediate_to_current .= state_space_variance_derivative[:,[intermediate_time_index,
                                                                                                      total_number_of_states+intermediate_time_index],
                                                                                                     [current_time_index,
                                                                                                      total_number_of_states+current_time_index]]
                 # This corresponds to d_P(s,t-tau)/d_theta
            covariance_matrix_derivative_intermediate_to_past .= state_space_variance_derivative[:,[intermediate_time_index,
                                                                                                    total_number_of_states+intermediate_time_index],
                                                                                                   [past_time_index,
                                                                                                    total_number_of_states+past_time_index]]
            ...
            # Fill in the big matrix
            state_space_variance_derivative[:,[intermediate_time_index,
                                               total_number_of_states+intermediate_time_index],
                                              [next_time_index,
                                               total_number_of_states+next_time_index]] .= covariance_matrix_derivative_intermediate_to_next
            # transpose arguments
            state_space_variance_derivative[:,[next_time_index,
                                               total_number_of_states+next_time_index],
                                              [intermediate_time_index,
                                               total_number_of_states+intermediate_time_index]] .= permutedims(covariance_matrix_derivative_intermediate_to_next,[1,3,2])
...
function kalman_update_step(state_space_mean,
    ...
    # consistent with variance (P)
    stacked_state_space_mean_derivative = Array{Float64}(undef,(7,2*(discrete_delay+1)));
    ...
    adaptation_coefficient_derivative = Array{Float64}(undef,(7,length(all_indices_up_to_delay)));#zeros(7,length(all_indices_up_to_delay))
    for parameter_index in 1:7
    ...
    updated_stacked_state_space_mean_derivative = Array{Float64}(undef,(7,2*(discrete_delay+1)));#zeros(7,2*(discrete_delay+1))
    for parameter_index in 1:7
    ...
    # unstack the rho into two columns, one with mRNA and one with protein
    updated_state_space_mean_derivative = Array{Float64}(undef,(discrete_delay+1,7,2));#zeros((discrete_delay+1),7,2)
    for parameter_index in 1:7
    ...
    # This is d_P*/d_theta
    updated_shortened_covariance_derivative_matrix = Array{Float64}(undef,(7,length(all_indices_up_to_delay),length(all_indices_up_to_delay)));#zeros(7,length(all_indices_up_to_delay),length(all_indices_up_to_delay))
    for parameter_index in 1:7
